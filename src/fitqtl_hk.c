               ...
               double *design_mat, int *matrix_rank, double *residuals)
{
    ...
    /* allocate memory for working arrays, total memory is
       sizefull*n_ind+n_ind+4*sizefull for double array,
    ...
    lrss = log10( galtRssHK(pheno, n_ind, n_gen, n_qtl, Genoprob,
                            ...
                            design_mat, matrix_rank, residuals) );
                 ...
                 double *designmat, int *matrix_rank, double *residuals)
{
    ...
    double *work, *qty, *qraux, *coef, tol, sigmasq, **X;
    ...
    ******************************************************/
    ...
    F77_CALL(dqrls)(X[0], &n_ind, &sizefull, pheno, &ny, &tol, coef, residuals,
                    qty, &k, jpvt, qraux, work);
    /* on output, k contains the rank */
    *matrix_rank = k;

    /* calculate RSS */
    for(i=0; i<n_ind; i++) rss_full += residuals[i]*residuals[i];
